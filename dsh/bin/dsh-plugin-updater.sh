...
staging_test() {  # $1 = pkg@newver
    ...
    for i in $(seq 1 15); do
        ...
        if [[ "$code" =~ ^(200|302|401)$ ]]; then ok=1; break; fi
...
rollback() {
    ...
    log "回滚后 HTTP: $code"
    [[ "$code" =~ ^(200|302|401)$ ]] && log "✅ 已恢复" || log "❌ 仍需人工处理（dp-status / 查看日志）"
}
...
if [[ -n "$(ls "$SNAP" 2>/dev/null)" ]]; then
    if [[ "${DELAY_RESTART:-0}" == "1" ]]; then
        ...
        log "  重启已调度，健康检查延迟进行"
        # 延迟健康检查（重启后 12s 检查）
        nohup bash -c "sleep 15; code=\$(curl -s -m 5 -o /dev/null -w '%{http_code}' http://127.0.0.1:3080 2>/dev/null); if echo \"\$code\" | grep -qE '200|302|401'; then echo \"[$(date '+%H:%M:%S')] 重启后健康 HTTP \$code\" >> $LOG; echo \"## 结果：✅ 重启后健康（HTTP \$code）\" >> $REPORT; else echo \"[$(date '+%H:%M:%S')] ❌ 重启后不健康 HTTP \$code\" >> $LOG; echo \"## 结果：❌ 重启后不健康（HTTP \$code）—— 需回滚 \$SNAP\" >> $REPORT; fi" > /dev/null 2>&1 &
    else
        log "🔄 重启 dsh web..."
        launchctl kickstart -k "gui/$(id -u)/$LABEL"
        sleep 8
