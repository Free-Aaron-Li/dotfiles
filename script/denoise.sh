...
# 定义日志文件路径，存储在脚本所在目录下
LOG_FILE="$(pwd)/denoise.log"

# 清空或创建日志文件
touch "$LOG_FILE"

# 使用 find 命令递归遍历当前目录下所有扩展名为 mp4 的文件
while IFS= read -r -d '' file; do
	# 获取文件所在的目录
	dir_path=$(dirname "$file")

	# 生成临时输出文件名，通过去掉原文件扩展名并添加 "_temp.mp4" 实现
	temp_output="${dir_path}/${file%.*}_temp.mp4"
	...
	if [ $? -eq 0 ]; then
		...
		mv -f "$temp_output" "$abs_file_path"

		# 将成功处理的文件的绝对路径写入日志文件
		echo "$abs_file_path" >>"$LOG_FILE"

		# 输出处理成功的提示信息
		echo "✅ 已处理: $abs_file_path"
	else
		# 如果命令执行失败，则删除生成的临时文件
		rm -f "$temp_output"

		# 输出处理失败的提示信息
		echo "❌ 处理失败: $abs_file_path"
	fi
done < <(find . -type f -name "*.mp4" -print0)

# 输出处理完成的提示信息
echo "处理完成！日志文件已生成：$LOG_FILE"
